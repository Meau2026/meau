...
import { addDoc, arrayUnion, collection, doc, GeoPoint, getDoc, getDocs, serverTimestamp, setDoc, query, where, updateDoc, deleteDoc } from 'firebase/firestore';
...
const DUMMY_INTERESSADOS: Interessado[] = [
  {
    id: 'int_01',
    userId: 'user_123',
    userName: 'CARLOS SILVA',
    userFoto: 'https://placehold.co/150.png',
    petId: 'pet_abc',
    petName: 'BÍDU',
  },
  {
...
export default function InteressadosScreen() {
  ...
  const [listaInteressados, setListaInteressados] = useState<Interessado[]>(DUMMY_INTERESSADOS);
  ...
  useEffect(() => {
    ...
    // Busca apenas os interesses onde o usuário atual é o DONO do pet
...
const handleRejeitar = (item: Interessado) => {
  Alert.alert(
    ...
    [
      ...
      { 
        ...
        onPress: async () => {
          try {
            // 1 e 2. Acessa o doc do animal e adiciona o user aos 'interesses_negados'
            const petRef = doc(db, 'animais', item.petId);
            await updateDoc(petRef, {
              interesses_negados: arrayUnion(item.userId)
            });

            // 3. Remove o pedido da tabela de interesses
            const interesseRef = doc(db, 'interesses', item.id);
            await deleteDoc(interesseRef);

            // Nota: O setListaInteressados manual foi removido daqui! 
            // O onSnapshot do useEffect vai atualizar a tela sozinho assim que o deleteDoc terminar.
  ...
  try {
    ...
    //Verifica se ja existe um chat entre o dono e este interessado sobre este pet
    const q = query(
      chatsRef,
    ...
    let tempoMs = Date.now(); // Padrão: momento atual
    ...
    // 4. Navega para a tela passando o objeto stringificado
    router.push({
      pathname: '/chat/chat',
  ...
  const renderInteressado = ({ item }: { item: Interessado }) => (
    <View style={styles.cardContainer}>
      ...
      <View style={styles.actionRow}>
        ...
        <TouchableOpacity 
          style={[styles.actionButton, styles.btnAceitar]} 
        ...
        <TouchableOpacity 
          style={[styles.actionButton, styles.btnRejeitar]} 
        ...
        <TouchableOpacity 
          style={[styles.actionButton, styles.btnChat]} 
...
const styles = StyleSheet.create({
  ...
  actionRow: {
    ...
    gap: 8, // O gap distribui os 3 botões perfeitamente
  },
  actionButton: {
    flex: 1,
    height: 36,
  ...
  // Cores seguindo a paleta do Meau
  btnChat: {
    backgroundColor: '#cfe9e5', // Ciano claro
  },
  btnRejeitar: {
    backgroundColor: 'transparent',
    borderWidth: 1,
    borderColor: '#bdbdbd', // Borda cinza
  },
  btnAceitar: {
    backgroundColor: '#fdcf58', // Amarelo
  },
